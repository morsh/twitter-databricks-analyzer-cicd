...
from modeling.models.trend_detector import *


class StlTrendinessDetector(AbstractTrendDetector):
    __name__ = 'StlTrendinessDetector'
    """An detector for anomalies on time series using the STL method (Seasonal Trend using Loess)
    ...
    def fit_one_category(self, dataset, category=None, verbose=False):
        ...
        results['residual_anomaly'] = np.where(residual > (results['residual_median']  + (results['residual_std']  * self.num_of_std)), 1, 0)
        results['trend_anomaly'] = np.where(trend > results['trend_median'] + (results['trend_std']  * self.num_of_std), 1, 0)
